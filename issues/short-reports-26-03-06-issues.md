...
## 2. [TSCC] Padronizar orquestração Docker (docker-compose na raiz) e documentação de execução
...
## 3. [TSDQ] Diagnosticar perda de dados em CSV (Excel vs geração/extração)
...
## 4. [TSDQ] Adaptar interface para nova leitura/ingestão de dados (incl. monitoramento de corrente)
...
## 5. [TSRE] Resolver execução de notebooks (.ipynb) no VSCode para exemplos do OpenDER
...
## 6. [TSRE] Evoluir dashboard Streamlit para comparação de simuladores FV (OpenDSS vs simulador próprio)
...
## 7. [PADE] Adaptar exemplos `tests/v1` para Python 3.12.11 (pade2026)
...
## 8. [PADE] Investigar/corrigir resíduos hexadecimais no sniffer (`events.csv`) e validar encoding
...
## 9. [PADE] Preparar Release Candidate do PADE para Python 3.12.11
